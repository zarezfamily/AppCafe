...
export default function AppDialogModal({ visible, onClose, title, description, actions = [] }) {
  ...
  const isNeutralLabel = (label) =>
    /(^|\s)(cancelar|cerrar|volver|atras|atrás)(\s|$)/i.test(
      String(label || '')
        .trim()
        .toLowerCase()
    );
  ...
  return (
    <Modal visible={!!visible} transparent animationType="fade" onRequestClose={onClose}>
      <View
        style={{
          flex: 1,
          backgroundColor: 'rgba(16, 10, 7, 0.62)',
      ...
      >
        ...
        >
          ...
          <Text style={{ fontSize: 10, fontWeight: '800', letterSpacing: 1.4, color: '#d4a574' }}>
            ETIOVE SETTINGS
          </Text>
          <Text style={{ fontSize: 18, fontWeight: '900', color: '#fff8f0' }}>{title}</Text>
          {!!description && (
            <Text style={{ fontSize: 13, lineHeight: compactActions ? 18 : 20, color: '#f4dfc8' }}>
              {description}
            </Text>
          )}
            ...
            {orderedActions.map((action, idx) =>
              (() => {
                ...
                return (
                  <TouchableOpacity
                    ...
                    style={{
                      ...
                      width: compactActions
                        ? orderedActions.length % 2 === 1 && idx === orderedActions.length - 1
                          ? '100%'
                          : '48.5%'
                        : '100%',
                  ...
                  >
                    <Text style={{ color: colors.textColor, fontSize: 13, fontWeight: '800' }}>
                      {action.label}
                    </Text>
                  </TouchableOpacity>
                );
