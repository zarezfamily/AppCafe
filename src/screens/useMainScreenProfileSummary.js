...
export default function useMainScreenProfileSummary({
  ...
  const unlockedAchievements = achievementDefs.filter((achievement) => gamification.achievementIds.includes(achievement.id));
  const pendingAchievements = achievementDefs.filter((achievement) => !gamification.achievementIds.includes(achievement.id));
