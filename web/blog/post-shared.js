...
(function () {
  ...
  const renderComments = (list, listEl) => {
    if (!listEl) return;
    if (!list.length) {
      listEl.innerHTML = '<p style="font-size:14px;color:#9a7963;margin-top:8px;">Sé el primero en comentar este artículo.</p>';
      return;
    }
  ...
  const initComments = async () => {
    ...
    sendBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        bodyEl.value = '';
        if (statusEl) { statusEl.textContent = '✓ Comentario publicado.'; statusEl.style.color = '#4f7a53'; }
        const updated = await getComments();
        renderComments(updated, listEl);
      } catch {
        if (statusEl) { statusEl.textContent = 'No se pudo publicar. Inténtalo de nuevo.'; statusEl.style.color = '#a44f45'; }
      } finally {
        sending = false;
