...
export default function CommunityModalActions({
  ...
  return (
    <View style={{ flexDirection: 'row', gap: 10, marginTop: 14, paddingBottom: Platform.OS === 'ios' ? 8 : 14 }}>
      ...
      <TouchableOpacity style={[s.redBtn, { flex: 1, marginTop: 0 }]} onPress={onSubmit} disabled={!!loading}>
        {!!loading ? <ActivityIndicator color="#fff" /> : <Text style={s.redBtnText}>{submitLabel}</Text>}
      </TouchableOpacity>
    </View>
  );
}
