...
export default function MisCafesTab({
  ...
  return (
    <View style={{ paddingTop: 20 }}>
      ...
      {favCafes.length > 0 && (
        <View style={sectionCardStyle}>
          <View style={[s.sectionHeader, { marginTop: 0 }]}><Text style={s.sectionTitle}>⭐ Mis favoritos</Text></View>
      ...
      <View style={sectionCardStyle}>
        ...
        {hasQuery && (
          <>
            <View style={[s.sectionHeader, { marginTop: 12 }]}><Text style={s.sectionTitle}>Coincidencias en Cafés para ti</Text></View>
            ...
            {cafesParaTiFiltrados.length > 0 ? (
              <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingLeft: 16, paddingRight: 8, gap: 12 }}>
                {cafesParaTiFiltrados.map((item) => (
                  <CardHorizontal key={`pt-${item.id}`} item={item} badge={`${item.puntuacion}.0`} onPress={setCafeDetalle} favs={favs} onToggleFav={toggleFav} />
                ))}
              </ScrollView>
            ) : (
              <View style={{ paddingHorizontal: 16 }}><Text style={s.empty}>Sin resultados en Cafés para ti</Text></View>
            )}

            <View style={[s.sectionHeader, { marginTop: 18 }]}><Text style={s.sectionTitle}>Coincidencias en Mis favoritos</Text></View>
            <Text style={s.sectionSub}>{favoritosFiltrados.length} resultados</Text>
            {favoritosFiltrados.length > 0 ? (
              <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingLeft: 16, paddingRight: 8, gap: 12 }}>
                {favoritosFiltrados.map((item) => (
                  <CardHorizontal key={`fav-${item.id}`} item={item} badge={`${item.puntuacion}.0`} onPress={setCafeDetalle} favs={favs} onToggleFav={toggleFav} />
                ))}
              </ScrollView>
            ) : (
              <View style={{ paddingHorizontal: 16 }}><Text style={s.empty}>Sin resultados en favoritos</Text></View>
            )}

            <View style={[s.sectionHeader, { marginTop: 18 }]}><Text style={s.sectionTitle}>Coincidencias en Mi colección</Text></View>
