/*
  Importa cafes actuales de Cafe Saula / Cafes Aula a Firestore.

  Fuente de marca:
    https://www.cafesaula.com/es/cafes-premium

  Fuente de compra:
    https://www.cafesaula.com/store/es/

  Nota:
    Aula y Saula comparten el catalogo del store. Este importador deja
    la fuente separada como `aula` para mantener la marca independiente.
*/
...
const OUTPUT_JSON =
  process.env.OUTPUT_JSON || path.resolve(process.cwd(), 'scripts/cafe-import-aula-real.json');
...
async function main() {
  ...
  console.log(`[AULA] Categories: ${CATEGORY_URLS.length}`);
  console.log(`[AULA] Products discovered: ${productUrls.length} dry=${DRY_RUN}`);
  ...
  for (const url of productUrls) {
    ...
    cafes.push(cafe);
    console.log(`[AULA] Prepared: ${cafe.nombre}`);
  }

  fs.writeFileSync(OUTPUT_JSON, JSON.stringify(cafes, null, 2), 'utf8');
  console.log(`[AULA] Export JSON: ${OUTPUT_JSON} (${cafes.length} items)`);
  ...
  console.log(`[AULA] Done: ${written}/${cafes.length} (dry=${DRY_RUN})`);
}

main().catch((error) => {
  console.error('[AULA] Error:', error?.stack || error);
  process.exit(1);
});
