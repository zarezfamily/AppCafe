...
export default function QuizSection({
  ...
  useEffect(() => {
    SecureStore.getItemAsync(keyFavs).then((v) => v && setFavs(JSON.parse(v))).catch(() => {});
    SecureStore.getItemAsync(keyVotes).then((v) => v && setVotes(JSON.parse(v))).catch(() => {});
    SecureStore.getItemAsync(keyPrefs).then((v) => {
      if (v) {
      ...
      <View style={q.quizBox}>
        ...
        <View style={q.opcionesGrid}>
          {preguntaActual.opciones.map((op) => (
            <TouchableOpacity key={op.value} style={q.opcion} onPress={() => elegir(preguntaActual.id, op.value)} activeOpacity={0.8}>
...
const q = StyleSheet.create({
  ...
  startBtn: { backgroundColor: '#5d4030', borderRadius: 30, paddingHorizontal: 32, paddingVertical: 14, marginTop: 8 },
