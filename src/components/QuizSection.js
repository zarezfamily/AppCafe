...
function matchScore(cafe, prefs) {
  ...
  if (prefs.acidez && cafe.acidez) {
    ...
    if (
      prefs.acidez === 'alta' &&
      (a.includes('brillante') || a.includes('alta') || a.includes('viva'))
    )
      score += 2;
...
export default function QuizSection({ allCafes, onGamifyEvent }) {
  ...
  useEffect(() => {
    SecureStore.getItemAsync(KEY_FAVS)
      .then((v) => v && setFavs(JSON.parse(v)))
      .catch(() => {});
    SecureStore.getItemAsync(KEY_VOTES)
  ...
  const elegir = (qid, val) => {
    ...
    if (step < QUIZ.length) {
      ...
      calcular(np);
      SecureStore.setItemAsync(KEY_PREFS, JSON.stringify(np)).catch(() => {});
      setStep(5);
    }
  };
  ...
  return (
    <View>
      ...
      {resultados.length === 0 ? (
        <Text
          style={[
            { color: THEME.text.muted, textAlign: 'center', marginTop: 40, fontSize: 14 },
            { marginHorizontal: 16 },
          ]}
        >
